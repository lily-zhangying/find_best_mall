...
class cf(recsys.recsys):
    ...
    def predict_for_user(self, user_ratings, user_feat, k, feature_transform_all =None):
        ...
        if (feature_transform_all == None):
            ...
            self.feature_transform = self.feature_helper(np.concatenate((self.X, user_ratings)), np.concatenate((self.user_feat, user_feat)))
        ...
        S=S.reshape((Nusers, 1)) #create a column vector
        ...
        result = np.argsort(predicted_values) #predict top results
        return result[0:k]
    ...
    def fit(self, train_indices = None, test_indices = None):
        ...
        self.X_predict[self.X_train == 1] =1 #compute X_predict fast, slow version is below
...
nmf = nmf_helper(2)
X= np.array([[0, 1, 1], [0, 1, 1], [1, 0, 0]]).T
...
test.predict_for_user(np.array([[0, 1, 0]]), np.array([[7, 7, 7, 0, 0, 0, 0]]), 1)